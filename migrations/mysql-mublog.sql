...
CREATE TABLE IF NOT EXISTS `entities_article_data` (
  ...
  `views` smallint(5) NOT NULL DEFAULT '0' COMMENT 'Number of article views',
  `rating` SMALLINT(5) NOT NULL DEFAULT '0' COMMENT 'Article voting rating',
  PRIMARY KEY (`eid`) USING BTREE,
  UNIQUE KEY `alias` (`alias`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Дамп данных таблицы mublog.entities_article_data: ~0 rows (приблизительно)
INSERT INTO `entities_article_data` (`eid`, `title`, `summary`, `body`, `alias`, `status`, `preview_src`, `preview_alt`, `author`, `views`, `rating`) VALUES
	(1, 'Набор полезных навыков для веб-разработчика от Андреаса Мэлсена', 'Веб-разработчик из Германии Андреас Мэлсен создал сайт, на котором он собрал множество полезных инcтрументов для веб-разработчиков и поделился с нами.', '<p>Веб-разработчик из Германии <a href="https://andreasbm.github.io/" target="_blank">Андреас Мэлсен</a> создал очень полезный сайт, <a href="https://andreasbm.github.io/web-skills/" target="_blank">Web Skills</a>, на котором он собрал и представил множество полезных ссылок на документации и статьи для изучения различных инструментов, которые могут очень пригодиться и быть полезными в веб-разработке.</p> <a href="https://andreasbm.github.io/web-skills/" target="_blank"> <img src="/images/content/220210-image-web-skills-demo.gif" alt="Web Skills Demo" width="800" height="512"> </a> <p>На данном сайте выполнено визуальное представление полезных навыков по мнению Андреаса. Он рассказывает, что собранные им инструменты - результат 10-летнего опыта в веб-разработке и с которыми он, так или иначе, сталкивался лично. Новичкам же автор не рекомендует воспринимать свою библиотеку знаний, как руководство к изучению всего, что там представлено, а воспринимать это скорее как то, что возможно им придётся изучить в будущем. Так же Андреас утверждает, что на его сайте собрано гораздо больше, чем может потребоваться в повседневной разработке проектов, по этому не стоит пугаться столь огромного количества представленных материалов.</p>', 'nabor-poleznih-navikov-dlya-veb-razrabotchika-ot-andreasa-melsena', 1, '/images/content/220210-preview-01.webp', 'Web Skills', 'mublog.site', 0, 0);
